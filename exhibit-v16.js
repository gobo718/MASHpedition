(()=>{
  ...
  function setView(view){
    const allowed=new Set(['door','left','wall','right','overhead','zoom','thumbnail']);
  ...
  // ART 22 | DOOR 1 | ART 1, with Door 1 centered on the straight wall.
  frontDoor.addEventListener('click',()=>renderRoom(23));

  doorBtn.addEventListener('click',()=>{
    // Return to the exterior/front-door surface.
    roomStart=23;
    lastRoomStart=23;
    setView('door');
  });
  ...
  // Prepare the complete overhead map, then start outside facing Door 1.
  overheadSlots.forEach(button=>{
    const index=Number(button.dataset.roomIndex);
    paintOverheadSlot(button,roomSlots[wrap(index)]);
  });
